#!/bin/bash
if ! [[ -e /home/jenkins/.jenkins/maven ]]; then
#  /usr/share/maven/conf/settings.xml.template
  if ! [[ -z "$NEXUS_USER" || -z "$NEXUS_PASSWORD" || -z "$NEXUS_SNAPSHOT_REPO_URL" || -z "$NEXUS_RELEASE_REPO_URL" ]]; then
    ...
    sudo cp /home/jenkins/.jenkins/settings.xml.template /usr/share/maven/conf/settings.xml.new
  ...
  touch /home/jenkins/.jenkins/maven
else
  echo "INFO: Maven has been already configured and this Agent Node!!"
fi
exit 0
