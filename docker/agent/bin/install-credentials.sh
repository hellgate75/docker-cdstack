...
if ! [[ -e $JENKINS_HOME/.jenkins/.ssh_remote ]]; then
  if ! [[ -z "$SSH_KEY_FILES_TAR_GZ_URL" ]]; then
    download_file "$JENKINS_HOME/remote-jenkins-ssh.tgz" "$SSH_KEY_FILES_TAR_GZ_URL"
    if [[ -e $JENKINS_HOME/remote-jenkins-ssh.tgz ]]; then
      echo "Installing new keys from archive, via URL : $SSH_KEY_FILES_TAR_GZ_URL"
      tar -xzf $JENKINS_HOME/remote-jenkins-ssh.tgz -C $JENKINS_HOME/.ssh
      rm -f $JENKINS_HOME/remote-jenkins-ssh.tgz
